...
public class ReflectiveCommandDispatcher implements CommandDispatcher {
                ...
                for (Method m : cmClass.getMethods()) {
                    ...
                    try {
                        if (params.length == 2 && params[0] == String.class) {
                            ...
                            m.invoke(commandManager, command, createServerHandlerProxy(handler));
    ...
    private Object createServerHandlerProxy(CommandHandler handler) {
        ...
        return java.lang.reflect.Proxy.newProxyInstance(getClass().getClassLoader(), new Class<?>[]{BiConsumer.class}, (proxy, method, args) -> {
            if ("accept".equals(method.getName()) && args != null && args.length == 2) {
                ...
                return "Proxy for " + handler.toString();
            }
            return null;
        });
    }
        ...
        for (Method m : defClass.getMethods()) {
            if (m.getName().toLowerCase().contains("executor") || m.getName().toLowerCase().contains("handler")) {
                try {
                    ...
                    m.invoke(def, createServerHandlerProxy(handler));
